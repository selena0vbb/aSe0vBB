...
def geometric_carrier_trapping(eventIDs):
    ...
    bmap = brewer2mpl.get_map(
        "Set2", "Qualitative", max(len(eventIDs) + 1, 3)
    ).mpl_colors
...
def noise_histogram():
    ...
    fig, ax = plt.subplots()
    ax.hist(
        energy, bins=100, range=(100, 140), histtype="step", linewidth=2, color=bmap[0]
    )
...
def noise_histogram_multiple_events():
    ...
    for j in range(len(plotTitle)):
        ...
        for i, event in enumerate(eventCollection.collection):
            ...
            if round(etot) == 122:
                if np.max(np.abs(flatData["y"])) < 0.5 and zmin > -0.05:
                    ...
                    # indx1us = np.where(t > 1)[0][0]
                    # indx20us = t.size - 1
                    q1, q20 = charge_compute_wrapper(
                        event, emfilename[j], e, wehp, neg[j], **settings
                    )
        ...
        ax.set_title(
            "Charge Induced at %s  for Different 122 keV Events" % plotTitle[j],
            fontsize=16,
        )
        ax.set_xlabel("Energy (keV)", fontsize=14)
        ax.legend(loc=2)
