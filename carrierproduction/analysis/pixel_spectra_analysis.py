...
def compare_partial_particle_tracks():
    ...
    noPartialPattern = [
        "pixel_particle_event_all.npy",
        "pixel_particle_event_all_poisson_trap.npy",
    ]

    ePartial = pa.getEnergySpectrum(pa.readBatchFiles(outdirPartial, partialPattern[0]))
    eNoPartial = pa.getEnergySpectrum(
        pa.readBatchFiles(outdirNoPartial, noPartialPattern[0])
    )

    ePartialPT = pa.getEnergySpectrum(
        pa.readBatchFiles(outdirPartial, partialPattern[1])
    )
    eNoPartialPT = pa.getEnergySpectrum(
        pa.readBatchFiles(outdirNoPartial, noPartialPattern[1])
    )
    ...
    fig.suptitle(
        "Comparison of Spectrum when including/excluding Partial Tracks", fontsize=18
    )
...
def higher_statistics_spectra():
    ...
    ax.hist(
        ...
        linewidth=3,
        color=bmap[1],
        label="Poisson + Trapping",
    )
    ax.set_yscale("log")
    ax.legend(loc="upper left", fontsize=16)
    ax.set_xlabel("Energy (keV)", fontsize=16)
    ax.set_title(
        "Higher Statistics (N=%i) Energy Spectra 122 keV" % len(e122), fontsize=18
    )
...
def new_pixel_particle_spectra():
    ...
    e122pt = pa.getEnergySpectrum(pa.readBatchFiles(outdir, pattern[1]))
    e122ptSimple = pa.getEnergySpectrum(
        pa.readBatchFiles(
            r"C:\Users\alexp\Documents\UW\Research\Selenium\Coplanar Detector\sim_data\pixel_detector\122keV\simple",
            "pixel_122kev_400k_pt\d+.npy",
        )
    )
    ...
    e136pt = pa.getEnergySpectrum(pa.readBatchFiles(outdir136, pattern136[1]))
    e136ptSimple = pa.getEnergySpectrum(
        pa.readBatchFiles(
            r"C:\Users\alexp\Documents\UW\Research\Selenium\Coplanar Detector\sim_data\pixel_detector\136keV\simple",
            "pixel_136kev_400k_pt\d+.npy",
        )
    )
...
def add_noise_to_pulses(outdir, pattern):
    ...
    adcConversion = 0.05 / 1.6e-19 * 750.0 / 122.0
    ...
    for i in range(data.shape[0]):
        ...
        data[i][1] = np.concatenate(
            [appendTime, data[i][1], np.min(data[i][1]) * np.ones(len(appendTime))]
        )
...
def noise_spectrum():
    ...
    e122NoNoise = pa.readBatchFiles(
        r"C:\Users\alexp\Documents\UW\Research\Selenium\Coplanar Detector\sim_data\pixel_detector\122keV\sio2",
        "pixel_122kev_sio2_5M_pt\d+.npy",
    )
    e136NoNoise = pa.readBatchFiles(
        r"C:\Users\alexp\Documents\UW\Research\Selenium\Coplanar Detector\sim_data\pixel_detector\136keV\sio2",
        "pixel_136kev_sio2_5M_pt\d+.npy",
    )
    # Get energy spectrum with no noise
    e122NoSpec = pa.getEnergySpectrum(e122NoNoise)
    e136NoSpec = pa.getEnergySpectrum(e136NoNoise)

    e122 = add_noise_to_pulses(
        r"C:\Users\alexp\Documents\UW\Research\Selenium\Coplanar Detector\sim_data\pixel_detector\122keV\sio2",
        "pixel_122kev_sio2_5M_pt\d+.npy",
    )
    e136 = add_noise_to_pulses(
        r"C:\Users\alexp\Documents\UW\Research\Selenium\Coplanar Detector\sim_data\pixel_detector\136keV\sio2",
        "pixel_136kev_sio2_5M_pt\d+.npy",
    )
...
def compare_angled_spectrum(logplot=False):
    ...
    obliquedata = pa.readBatchFiles(
        r"C:\Users\alexp\Documents\UW\Research\Selenium\Coplanar Detector\sim_data\pixel_detector\122keV\sio2\Efield",
        "pixel_122kev_sio2_4000V_75deg_200k_pt\d+.npy",
    )
    ...
    # Plot spectra
    normhist, binx, _ = ax0.hist(
        np.array(normalSpectrum),
        bins=bins,
        density=True,
        histtype="step",
    ...
    obliquehist, _, _ = ax0.hist(
        np.array(obliqueSpectrum),
        bins=bins,
        density=True,
        histtype="step",
        label="Oblique Incidence",
        linewidth=2,
        color="red",
        log=logplot,
    )
    ...
    ax0.set_title(
        "Pixel Detector Spectra with Different Angle of Incidence", fontsize=14
    )
    ...
    # Plot residual
    ax1.plot(
        xaxis, normhist - obliquehist, linewidth=3, color="black", label="residual"
    )
    ax1.fill_between(
        xaxis,
        normhist - obliquehist - (errNormal + errOblique),
        normhist - obliquehist + (errNormal + errOblique),
        color="black",
        alpha=0.5,
...
def compare_pulse_pixel2coplanar():
    ...
    pixelPulse = pa.writeBinFiles(
        ...
        filterPulse=True,
        returnPulse=True,
        returnPulseIdx=pindx,
    )

    coplanarPulse = pa.writeBinFiles(
        "test.dat",
        coplanarFilepath,
        coplanarPattern,
        minEnergy=100,
    ...
    ax1.plot(
        t[int(tstart / dt) : int(tend / dt)][::ds],
        scp.decimate(pixelPulse[int(tstart / dt) : int(tend / dt)] - 3600, ds),
        linewidth=2,
        color="black",
        label="Single Pixel Simulated",
    )
    ax1.plot(
        t[int(tstart / dt) : int(tend / dt)],
        -800 * np.ones(t.size)[int(tstart / dt) : int(tend / dt)],
        "--r",
        linewidth=3,
        label="Expected Pulse Height",
    )
    ...
    ax2.plot(
        t[int(tstart / dt) : int(tend / dt)][::ds],
        scp.decimate(coplanarPulse[int(tstart / dt) : int(tend / dt)] - 3600, ds),
        linewidth=2,
        color="black",
        label="Coplanar Simulated",
    )
    ax2.plot(
        t[int(tstart / dt) : int(tend / dt)],
        -800 * np.ones(t.size)[int(tstart / dt) : int(tend / dt)],
...
def plot_noise_vs_no_noise():
    ...
    data = pa.readBatchFiles(
        r"C:\Users\alexp\Documents\UW\Research\Selenium\Coplanar Detector\sim_data\pixel_detector\122keV\sio2",
        "pixel_122kev_sio2_5M_pt\d+.npy",
    )
    dataNoise = add_noise_to_pulses(
        r"C:\Users\alexp\Documents\UW\Research\Selenium\Coplanar Detector\sim_data\pixel_detector\122keV\sio2",
        "pixel_122kev_sio2_5M_pt\d+.npy",
    )
    energyConversion = 0.05 / 1.6e-19
