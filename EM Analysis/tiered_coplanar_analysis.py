...
phi, Ey, Ez = data[:,0:np.amin(dataTier.shape):3]/1e6, data[:,1:np.amin(dataTier.shape):3]/1e6, data[:,2:np.amin(dataTier.shape):3]
...
for i in range(np.amin(Ey.shape)):
	...
	EyInter = scp.interp2d(yTier, zTier, np.reshape(Ey[:,i],(zTier.size, yTier.size)), kind='linear') 
	EzInter = scp.interp2d(yTier, zTier, np.reshape(Ez[:,i],(zTier.size, yTier.size)), kind='linear')

	Enorm = []	
	for j in range(yi.size):
	
		Enorm.append(np.sqrt(EyInter(yi[j], zi)**2 + EzInter(yi[j], zi)**2)) # Keep track of the magnitude of the efield at different points
