...
E = [
    ...
    data[:, 1] / 1e6,
    data[:, 2] / 1e6,
]  # Setting up the E field input to finding the motion. Convert E to V/um
...
for i in range(N):
    initialPos = (
        np.random.uniform(1.1 * yrange[0], 0.9 * yrange[1]),
        np.random.uniform(zrange[0], 0.5 * zrange[1]),
    )
    particleMotion = findMotion(initialPos, E, vDriftHoles, dt, q=1.6e-19)
    try:
        qa, qb, qdiff = inducedCharge(
