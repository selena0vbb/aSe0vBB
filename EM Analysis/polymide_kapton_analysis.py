...
def effectiveAreaOfDifferentKaptonWidth():
	...
	kaptonThickness = {'500nm':0.5, "1um":1, "2um":2, "5um":5, "10um":10, "15um":15, "3um":3, "4um":4}
	legend = [] 
	...
	for findx, file in enumerate(filelist):
		...
		for i in range(np.amin(phi.shape)):
			...
			E = [ y, z, Ey[:,i]/1e6, Ez[:,i]/1e6 ] # building the E object and converting e fields to V/um

			# Iterate over different y values
			for j in range(ystart.size):
				path = findMotion((ystart[j], zstart), E, vDriftHoles, dt, q=1.6e-19, limits=boundaryLimits)

				# check if last value falls in limits for electrode A
				for k in range(nElectrodes):
					if (path[-1,0] > electrodeABoundary[2*k] and path[-1,0] < electrodeABoundary[2*k+1]):
						qSign[j] = 1
	...
	ax.set_title('Effective Area vs Bias Potential for Different Width Kapton', fontsize=16)
...
def darkCurrentCalculation():
	...
	kaptonThickness = {'500nm':0.5, "1um":1, "2um":2, "5um":5, "10um":10, "15um":15, "3um":3, "8um":8}
	...
	for i, line in enumerate(ax.get_lines()):
		...
		ax.plot(bias[biasVoltageIndex[i]], darkCurrent[i, biasVoltageIndex[i]]*1e12, 'o', markersize=8, markeredgecolor=bmap[i], markeredgewidth=3, markerfacecolor="None")
